...
void image_grey(image_desc imgIn, image_desc *pimgOut)
{
  int i = 0, err = 1;
  err = mallocImage(pimgOut, imgIn.width, imgIn.height);
  if(!err){
  ...
  /* Transformation  : equiv. to : for (i=0; i < i_img.width*i_img.height; i++)*/
  for (i=0; i<(imgIn.width) * (imgIn.height); i++) {
    (pimgOut->pBlue)[i] = (imgIn.pBlue)[i]/3 + (pimgOut->pGreen)[i]/3 + (pimgOut->pRed)[i]/3;
    (pimgOut->pGreen)[i] = (imgIn.pBlue)[i]/3 + (pimgOut->pGreen)[i]/3 + (pimgOut->pRed)[i]/3;
    (pimgOut->pRed)[i] = (imgIn.pBlue)[i]/3 + (pimgOut->pGreen)[i]/3 + (pimgOut->pRed)[i]/3;
